    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (args.length < 1) { // why am i worried about negative argument length ? le-sigh
            ...
            if (pc.skillsSelected != null && !pc.skillsSelected.isEmpty()) {
                ...
                for (final Skill s : pc.skillsSelected) {
    ...
    private void sendSkillList(final CommandSender sender, final PC pc) {
        ...
        if (pc == null || !(sender instanceof Player)) {
            ...
            for (final ClassType cls : lq.classes.getClassTypes().values()) {
                for (final Skill s : cls.availableSkills) {
            ...
            final List<Skill> selected = pc.skillsSelected;

            for (final Skill s : pc.race.availableSkills) {
                strout = " - " + s.name + " [" + lq.configLang.statLevelShort + " " + s.levelRequired + " | " + lq.configLang.statSp + " " + s.skillPoints + "]";
            ...
            for (final Skill s : pc.mainClass.availableSkills) {
                strout = " - " + s.name + " [" + lq.configLang.statLevelShort + " " + s.levelRequired + " | " + lq.configLang.statSp + " " + s.skillPoints + "]";
                if (selected.contains(s)) {
                    strout += " *";
                }
            ...
            if (pc.subClass != null) {
                for (final Skill s : pc.subClass.availableSkills) {
                    strout = " - " + s.name + " [" + lq.configLang.statLevelShort + " " + s.levelRequired + " | " + lq.configLang.statSp + " " + s.skillPoints + "]";
                    if (selected.contains(s)) {
                        strout += " *";
                    }
