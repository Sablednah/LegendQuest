...
public class SkillLoader {
    ...
    private void load(final ClassLoader loader, final LinkedList<SkillDefinition> skills, final String name, final String path) {
        ...
        if (clazz.isAnnotationPresent(SkillManifest.class)) {
            final SkillManifest manifest = (SkillManifest) clazz.getAnnotation(SkillManifest.class);
            if ((manifest.type() == null) || (manifest.author().equalsIgnoreCase("")) || (manifest.name().equalsIgnoreCase(""))) {
                System.out.println(name + " is not a valid skill and was ignored!");
                return;
            }
            final SkillDefinition def = new SkillDefinition(clazz, this, new SkillInfo(manifest.author(), manifest.name(), manifest.description(), manifest.type(), manifest.version(), manifest.concept(), manifest.requiredPowers()));
            skills.add(def);
            return;
        }
    }
    
    public void load(final File file, final LinkedList<SkillDefinition> defs, ClassLoader loader)
            throws IOException
    {
